...
async def main():
    ...
    with freeze_time(datetime.now()) as frozen_datetime:
        ...
        for i in range(24):
            ...
            if i == 0:
                data["res"], data["history"] = start_genetic_algorithm(data=data, pop_size=10, n_gen=10, n_threads=24, sampling=None, verbose=False)  #Checked OK
            else:
                data["res"], data["history"] = start_genetic_algorithm(data=data, pop_size=10, n_gen=10, n_threads=24, sampling=sampling, verbose=False)
            ...
            update_battery_value(data, "csv/socs.csv", dict[f"b{i}"], dict[f"i{i}"])
